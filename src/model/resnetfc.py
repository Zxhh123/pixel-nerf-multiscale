...
class ResnetFC(nn.Module):
    def __init__(
        ...
        # ==================== ✅ 修复：类型转换，确保所有参数都是整数 ====================
        # Ensure all parameters are integers (not lists or tuples)

        # ✅ 对于 d_in：如果是列表/元组，取第一个元素
        if isinstance(d_in, (list, tuple)):
            d_in = d_in[0] if len(d_in) > 0 else 3

        # ✅ 对于 d_out：如果是列表/元组，取第一个元素
        if isinstance(d_out, (list, tuple)):
            d_out = d_out[0] if len(d_out) > 0 else 4

        # ✅ 对于 d_latent：如果是列表/元组，求和（这是关键修复！）
        if isinstance(d_latent, (list, tuple)):
            d_latent = sum(int(x) for x in d_latent) if len(d_latent) > 0 else 0
            print(f"✅ ResnetFC: d_latent is list/tuple, summed to {d_latent}")

        # ✅ 对于 d_hidden：如果是列表/元组，取第一个元素
        if isinstance(d_hidden, (list, tuple)):
            d_hidden = d_hidden[0] if len(d_hidden) > 0 else 128

        # ✅ 对于 n_blocks：如果是列表/元组，取第一个元素
        if isinstance(n_blocks, (list, tuple)):
            n_blocks = n_blocks[0] if len(n_blocks) > 0 else 5

        # ✅ 对于 combine_layer：如果是列表/元组，取第一个元素
        if isinstance(combine_layer, (list, tuple)):
            combine_layer = combine_layer[0] if len(combine_layer) > 0 else 1000
        ...
        combine_layer = int(combine_layer)

        # ✅ 打印调试信息
        print(f"✅ ResnetFC initialized: d_in={d_in}, d_latent={d_latent}, d_hidden={d_hidden}, d_out={d_out}")
        # ====================================================================
    ...
    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        ...
        with profiler.record_function("resnetfc_infer"):
            # ✅ 添加调试信息
            expected_size = self.d_latent + self.d_in
            actual_size = zx.size(-1)
            if actual_size != expected_size:
                print(f"❌ Dimension mismatch in ResnetFC.forward:")
                print(f"   Expected: {expected_size} (d_latent={self.d_latent} + d_in={self.d_in})")
                print(f"   Actual: {actual_size}")
                print(f"   Input shape: {zx.shape}")

            assert zx.size(-1) == self.d_latent + self.d_in, \
                f"Input size {zx.size(-1)} != d_latent ({self.d_latent}) + d_in ({self.d_in}) = {self.d_latent + self.d_in}"
